    ...
    def para_renorm(self, para_t, para_v, atol):
        ...
        for i in range(len_paraind//2):
            ...
            for mat in mat_validbuffer:
                if srosignal_utils.mat_identical(mat, p, atol=atol):
                    sim_mat = True 
                    break
        ...
        if allow_virtual:
            ...
            for tri_i, bi_tri in enumerate(tri_mat):
                ...
                for virt in virt_vert_buffer_all:
                    if ((srosignal_utils.dot_in_tri(tri1, virt, tol_angle=tol_angle, tol_dis=tol_dis)) 
                        or (srosignal_utils.dot_in_tri(tri2, virt, tol_angle=tol_angle, tol_dis=tol_dis))):
    ...
    def draw_poly(self, bond_t, bond_v, c_bond):
        ...
        for bond_t, bond_v in zip(bond_t, bond_v):
            ...
            for bond in bond_buffer:
                if srosignal_utils.mat_identical(bond, mat_t, atol=self.atol):
                    sim_bond_t = True
                
                if srosignal_utils.mat_identical(bond, mat_v, atol=self.atol):
                    sim_bond_v = True
