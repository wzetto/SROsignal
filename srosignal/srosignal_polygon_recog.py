from srosignal import srosignal_utils 
import cv2
import numpy as np
...
class polygon_recog:
    def __init__(self,
        ...
        self.new_centre_list = srosignal_utils.centre_list_gen(inv_filter, stem_coords_threshold_pth, stem_coords_all_pth)
        self.dis_matrix = srosignal_utils.dis_mat_calc(mp_core, self.new_centre_list).mat_gen()
        self.img_raw = cv2.imread(img_raw_pth, cv2.IMREAD_GRAYSCALE)
        self.l_range = l_range
        self.centre_grids = srosignal_utils.grid_prepare(self.l_range, self.img_raw, self.new_centre_list)
    ...
    def recog_grid(self, count):
        ...
        for i, j, k in combinations(c_grid, 3):
            ...
            angle_list_temp = np.rad2deg([
                srosignal_utils.angle_calc(rj-ri, rk-ri),
                srosignal_utils.angle_calc(ri-rj, rk-rj),
                srosignal_utils.angle_calc(ri-rk, rj-rk),
            ])
            ...
            angle = srosignal_utils.angle_calc(self.new_centre_list[ijk_ind[bond_1[0]]]-self.new_centre_list[ijk_ind[bond_1[1]]], 
            ...
            for angle in [angle%np.pi, np.pi-angle%np.pi]:
                ...
                vertex_2nd = srosignal_utils.c2_dot(vertex_ind, bot_ind)
                ...
                if not self.allow_dot_in_pattern:
                    for i_dot in range(len(self.new_centre_list)):
                        ...
                        for tri_coords in [tri_coord_1, tri_coord_2]:
                            dot_in_tri_list.append(srosignal_utils.dot_in_tri(
                                                    tri_coords, 
